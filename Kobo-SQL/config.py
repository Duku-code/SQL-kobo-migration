...
class Config:
    ...
    def sqlalchemy_connection_string(self) -> str:
        ...
        if self.sql_server_auth_method.lower() in ("windows", "integrated", "trusted"):
            return (
                f"mssql+pyodbc://@{self.sql_server_host},{self.sql_server_port}"
                f"/{self.sql_server_database}?driver={driver}&Trusted_Connection=yes"
            )
...
def build_config(values: dict[str, Any] | None = None) -> Config:
    ...
    auth_method = _get_value(
        "SQL_SERVER_AUTH_METHOD", source=source, required=False, default="sql"
    ).lower()
    return Config(
        ...
        sql_server_user=_get_value(
            "SQL_SERVER_USER", source=source, required=(auth_method == "sql")
        ),
        sql_server_password=_get_value(
            "SQL_SERVER_PASSWORD", source=source, required=(auth_method == "sql")
        ),
        sql_server_auth_method=auth_method,
